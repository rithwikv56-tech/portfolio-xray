...
type Meta = { totalValue: string; confidence: "high" | "medium" | "low"; summary: string; verdict?: string; riskLevel?: RiskLevel; riskWhy?: string; persona?: string; personaLine?: string; riskScore?: number; growthScore?: number; reassure?: string; worstThing?: string; crashTest?: string };
...
export default function Home() {
  // Interactive ambient background — soft light follows the cursor (calm, subtle).
  useEffect(() => {
    let raf = 0;
  ...
  const [overlap, setOverlap] = useState<{ funds: string[]; text: string } | null>(null);
  ...
  // ---- Portfolio Memory (device-local, no accounts) ----
  // A compact snapshot of each completed analysis, stored only in this browser.
  type Snapshot = { at: number; totalValue: string; riskScore?: number; riskLevel?: string; persona?: string; topName?: string; topPct?: number };
  const [lastSnapshot, setLastSnapshot] = useState<Snapshot | null>(null);
  const savedThisRun = useRef(false);

  // When an analysis completes: load the previous snapshot (for comparison), then save this one.
  useEffect(() => {
    if (!done || !meta || holdings.length === 0 || savedThisRun.current) return;
    savedThisRun.current = true;
    // Top holding by weight
    let topName: string | undefined; let topPct = 0;
    for (const h of holdings) {
      const w = parseFloat(String(h.weight).replace(/[^0-9.]/g, ""));
      if (!isNaN(w) && w > topPct) { topPct = w; topName = h.name; }
    }
    const snap: Snapshot = {
      at: Date.now(), totalValue: meta.totalValue, riskScore: meta.riskScore,
      riskLevel: meta.riskLevel, persona: meta.persona, topName, topPct: topPct || undefined,
    };
    try {
      const prevRaw = window.localStorage.getItem("pxray-last-snapshot");
      if (prevRaw) {
        const prev = JSON.parse(prevRaw) as Snapshot;
        // Only surface the comparison if the previous run was a different day (avoids noise from re-runs)
        if (Date.now() - prev.at > 20 * 60 * 60 * 1000) setLastSnapshot(prev);
      }
      window.localStorage.setItem("pxray-last-snapshot", JSON.stringify(snap));
    } catch { /* private mode or storage unavailable — memory simply stays off */ }
  }, [done, meta, holdings]);
  ...
  function resetResults() {
    ...
    setNudge(""); setEquityBreakdown(null); setOverlap(null); setCost("");
    setLastSnapshot(null); savedThisRun.current = false;
  ...
  function handleLine(line: string, collectedNames?: string[]): string | null {
    ...
    switch (obj.kind) {
      ...
      case "overlap": setOverlap({ funds: Array.isArray(obj.funds) ? obj.funds.map(String) : [], text: String(obj.text ?? "") }); break;
      case "cost": setCost(String(obj.text ?? "")); break;
  ...
  async function analyze(overrideText?: string) {
    ...
    try {
      const res = await fetch("/api/analyze", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ statementText: sourceText, beginnerMode }),
      });
  ...
  return (
    <main style={{ minHeight: "100vh", padding: "clamp(16px, 3vw, 28px)", position: "relative" }}>
            ...
            <div className="tile" style={{ padding: 22, overflow: "hidden" }}>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
                ...
                <button
                  ...
                  title="Explains everything in the simplest possible language"
                  style={{ display: "inline-flex", alignItems: "center", gap: 6, padding: "4px 11px", borderRadius: "var(--r-pill)", border: `1px solid ${beginnerMode ? "var(--accent)" : "var(--line)"}`, background: beginnerMode ? "var(--accent-soft)" : "transparent", color: beginnerMode ? "var(--accent)" : "var(--ink-faint)", cursor: "pointer", fontSize: 12, fontWeight: 500, transition: "all .2s var(--ease)" }}
                >
                  <span style={{ width: 7, height: 7, borderRadius: "var(--r-pill)", background: beginnerMode ? "var(--accent)" : "var(--line)", transition: "background .2s var(--ease)" }} />
                  New to investing?
                </button>
              </div>
          ...
          <div>
            ...
            {hasResults && (
              <Results
                ...
                overlap={overlap} cost={cost} lastSnapshot={lastSnapshot}
              />
            )}
  ...
  return (
    <div className="tile fade-up" style={{ padding: "clamp(28px, 5vw, 52px)", display: "flex", flexDirection: "column", justifyContent: "center" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 16, marginTop: 32, flexWrap: "wrap" }}>
        <button onClick={onDemo} className="btn-primary btn-neon font-serif" style={{ borderRadius: 10, padding: "12px 22px", fontSize: 15, fontWeight: 600, cursor: "pointer" }}>
          See it with an example →
        </button>
        <span className="font-mono" style={{ fontSize: 11, color: "var(--ink-faint)", letterSpacing: 0.5 }}>Free · No signup · Nothing stored</span>
      </div>
    </div>
  );
}
...
function MemoryTile({ prev, meta, holdings }: { prev: { at: number; totalValue: string; riskScore?: number; topName?: string; topPct?: number }; meta: Meta; holdings: Holding[] }) {
  // Current top holding
  let topName: string | undefined; let topPct = 0;
  for (const h of holdings) {
    const w = parseFloat(String(h.weight).replace(/[^0-9.]/g, ""));
  ...
  const prevVal = parseValue(prev.totalValue);
  const curVal = parseValue(meta.totalValue);
  const days = Math.max(1, Math.round((Date.now() - prev.at) / (24 * 60 * 60 * 1000)));
  const when = days === 1 ? "yesterday" : days < 30 ? `${days} days ago` : days < 60 ? "about a month ago" : `${Math.round(days / 30)} months ago`;

  const lines: { icon: string; text: string }[] = [];
  if (prevVal > 0 && curVal > 0) {
    const diff = curVal - prevVal;
    const pct = Math.abs((diff / prevVal) * 100);
    if (pct >= 0.5) lines.push({ icon: diff > 0 ? "📈" : "📉", text: `Your portfolio value ${diff > 0 ? "grew" : "fell"} about ${pct.toFixed(0)}% since ${when}.` });
    else lines.push({ icon: "⚖️", text: `Your portfolio value is roughly unchanged since ${when}.` });
  }
  if (typeof prev.riskScore === "number" && typeof meta.riskScore === "number" && Math.abs(meta.riskScore - prev.riskScore) >= 5) {
    lines.push({ icon: meta.riskScore > prev.riskScore ? "🔺" : "🔻", text: `Your risk level moved ${meta.riskScore > prev.riskScore ? "up" : "down"} (${prev.riskScore} → ${meta.riskScore} on our 100-point scale).` });
  }
  if (prev.topName && topName && prev.topName === topName && prev.topPct && topPct && Math.abs(topPct - prev.topPct) >= 2) {
    lines.push({ icon: "🎯", text: `Your biggest holding (${topName}) went from ${prev.topPct.toFixed(0)}% to ${topPct.toFixed(0)}% of your portfolio${topPct > prev.topPct ? " — concentration is growing" : " — a bit more balanced now"}.` });
  } else if (prev.topName && topName && prev.topName !== topName) {
    lines.push({ icon: "🔄", text: `Your biggest holding changed: ${prev.topName} → ${topName}.` });
  }
  if (lines.length === 0) lines.push({ icon: "🧘", text: `Nothing major changed since ${when} — your portfolio looks steady.` });

  return (
    <div className="tile fade-up" style={{ marginBottom: "var(--sp-5)", padding: "18px 22px", borderColor: "var(--cyan)", background: "linear-gradient(135deg, #f4fbfd 0%, #edf7fa 100%)" }}>
      <div className="font-mono label-strong" style={{ fontSize: 10, color: "var(--cyan)", marginBottom: 10 }}>SINCE LAST TIME · REMEMBERED ON THIS DEVICE ONLY</div>
      <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
        {lines.map((l, i) => (
          <div key={i} style={{ display: "flex", gap: 10, alignItems: "flex-start" }}>
            <span style={{ flexShrink: 0, fontSize: 14 }}>{l.icon}</span>
            <span style={{ fontSize: 13.5, lineHeight: 1.5, color: "var(--ink)" }}>{l.text}</span>
          </div>
        ))}
      </div>
    </div>
...
function PersonaCard({ meta }: { meta: Meta }) {
  const [copied, setCopied] = useState(false);
  const [downloading, setDownloading] = useState(false);

  // Draws a clean shareable card to a canvas and downloads it as a PNG.
  function downloadCard() {
    if (!meta.persona) return;
    setDownloading(true);
    try {
      const W = 1080, H = 1350; // 4:5 — ideal for social feeds
      const cv = document.createElement("canvas"); cv.width = W; cv.height = H;
      const ctx = cv.getContext("2d");
      if (!ctx) { setDownloading(false); return; }
      // Background
      const g = ctx.createLinearGradient(0, 0, W, H);
      g.addColorStop(0, "#14352a"); g.addColorStop(0.55, "#1f7a52"); g.addColorStop(1, "#29a06a");
      ctx.fillStyle = g; ctx.fillRect(0, 0, W, H);
      // Dot texture
      ctx.fillStyle = "rgba(255,255,255,0.06)";
      for (let x = 24; x < W; x += 36) for (let y = 24; y < H; y += 36) { ctx.beginPath(); ctx.arc(x, y, 2.2, 0, Math.PI * 2); ctx.fill(); }
      // Label
      ctx.fillStyle = "rgba(255,255,255,0.75)"; ctx.font = "600 30px Courier, monospace"; ctx.textAlign = "center";
      ctx.fillText("MY PORTFOLIO PERSONALITY", W / 2, 200);
      // Persona (wrap if long)
      ctx.fillStyle = "#fffdf5"; ctx.font = "700 96px Georgia, serif";
      const words = (meta.persona ?? "").split(" ");
      let line = "", lines: string[] = [];
      for (const w_ of words) { const t = (line + " " + w_).trim(); if (ctx.measureText(t).width < W - 160) line = t; else { lines.push(line); line = w_; } }
      if (line) lines.push(line);
      lines.forEach((ln, i) => ctx.fillText(ln, W / 2, 340 + i * 110));
      let y = 340 + lines.length * 110 + 40;
      // Verdict (wrapped, smaller)
      if (meta.verdict) {
        ctx.fillStyle = "rgba(255,255,255,0.92)"; ctx.font = "400 40px Georgia, serif";
        const vw = meta.verdict.split(" "); let vl = ""; const vlines: string[] = [];
        for (const w_ of vw) { const t = (vl + " " + w_).trim(); if (ctx.measureText(t).width < W - 220) vl = t; else { vlines.push(vl); vl = w_; } }
        if (vl) vlines.push(vl);
        vlines.slice(0, 4).forEach((ln, i) => ctx.fillText(ln, W / 2, y + i * 54));
        y += Math.min(vlines.length, 4) * 54 + 60;
      }
      // Risk line
      if (meta.riskLevel) {
        ctx.fillStyle = "#22d3ee"; ctx.font = "600 34px Courier, monospace";
        ctx.fillText(`RISK: ${meta.riskLevel.toUpperCase()}`, W / 2, y);
      }
      // Footer
      ctx.fillStyle = "#fffdf5"; ctx.font = "700 44px Georgia, serif";
      ctx.fillText("Portfolio X-Ray", W / 2, H - 150);
      ctx.fillStyle = "rgba(255,255,255,0.7)"; ctx.font = "400 28px Arial, sans-serif";
      ctx.fillText("Understand what you actually own — free", W / 2, H - 100);
      ctx.fillText("portfolio-xray-steel.vercel.app", W / 2, H - 62);
      // Download
      const a = document.createElement("a");
      a.download = "my-portfolio-xray.png";
      a.href = cv.toDataURL("image/png");
      a.click();
    } catch { /* canvas unsupported — button simply does nothing harmful */ }
    setDownloading(false);
  }
  if (!meta.persona) return null;

  const shareText = `My portfolio personality: ${meta.persona} 🩻\n\n${meta.verdict ?? ""}\n\nFound out with Portfolio X-Ray — it explains your investments in plain English. Free: portfolio-xray-steel.vercel.app`;
  ...
  async function share() {
    try {
      ...
      try { await navigator.clipboard.writeText(shareText); setCopied(true); setTimeout(() => setCopied(false), 2000); } catch { /* no-op */ }
    }
  }

  return (
    <div className="tile fade-up" style={{ marginBottom: "var(--sp-5)", padding: 0, overflow: "hidden", border: "none" }}>
      <div style={{ position: "relative", padding: "clamp(24px, 4vw, 36px)", background: "linear-gradient(135deg, #1a3a2e 0%, #1f7a52 55%, #29a06a 100%)", color: "#fff" }}>
        <div style={{ position: "absolute", inset: 0, opacity: 0.08, backgroundImage: "radial-gradient(circle at 1px 1px, #fff 1px, transparent 0)", backgroundSize: "16px 16px" }} />
        <div style={{ position: "relative" }}>
          <div className="font-mono" style={{ fontSize: 10, letterSpacing: "0.14em", opacity: 0.75, marginBottom: 14 }}>YOUR PORTFOLIO PERSONALITY</div>
          <div className="font-serif" style={{ fontSize: "clamp(28px, 5vw, 42px)", fontWeight: 600, letterSpacing: "-0.02em", lineHeight: 1.05, marginBottom: 12 }}>{meta.persona}</div>
          {meta.personaLine && <p style={{ margin: 0, fontSize: 14.5, lineHeight: 1.5, opacity: 0.92, maxWidth: 460 }}>{meta.personaLine}</p>}
          <div style={{ display: "flex", alignItems: "center", gap: 14, marginTop: 22 }}>
            <button
              onClick={share}
              className="font-sans"
              style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "9px 16px", borderRadius: "var(--r-pill)", background: "#fff", color: "var(--accent)", border: "none", fontSize: 13, fontWeight: 600, cursor: "pointer" }}
            >
            ...
            <button
              ...
              className="font-sans"
              style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "9px 16px", borderRadius: "var(--r-pill)", background: "rgba(255,255,255,0.15)", color: "#fff", border: "1px solid rgba(255,255,255,0.4)", fontSize: 13, fontWeight: 600, cursor: "pointer" }}
            >
              {downloading ? "Preparing…" : "⬇ Download image"}
            </button>
            <span className="font-serif" style={{ fontSize: 13, fontWeight: 600, opacity: 0.85 }}>Portfolio X-Ray</span>
          </div>
        </div>
      </div>
    </div>
...
function CrashSlider({ totalValue, riskScore }: { totalValue: string; riskScore?: number }) {
  const [drop, setDrop] = useState(30);
  const total = parseValue(totalValue);
  if (total <= 0) return null;
  const risk = typeof riskScore === "number" ? riskScore : 60;
  const sensitivity = 0.35 + (risk / 100) * 0.7;
  const portfolioDrop = Math.min(drop * sensitivity, 95);
  const lost = Math.round((total * portfolioDrop) / 100);
  const remaining = total - lost;
  const fmt = (n: number) => "₹" + n.toLocaleString("en-IN");
  return (
    <div className="tile reveal col-6" style={{ padding: "22px 24px", background: "linear-gradient(135deg, #1a1a17 0%, #28241f 100%)", border: "none" }}>
      <div className="font-mono" style={{ fontSize: 10, letterSpacing: "0.12em", color: "rgba(255,255,255,0.55)", marginBottom: 4 }}>STRESS TEST · DRAG TO SEE</div>
      <p className="font-serif" style={{ margin: "0 0 18px", fontSize: 17, fontWeight: 600, color: "#fff" }}>If the market fell <span style={{ color: "var(--cyan-bright)" }}>{drop}%</span>…</p>
      <input type="range" min={5} max={60} step={5} value={drop} onChange={(e) => setDrop(Number(e.target.value))} style={{ width: "100%", accentColor: "var(--cyan-bright)", cursor: "pointer", marginBottom: 18 }} aria-label="Market crash percentage" />
      <div style={{ display: "flex", gap: 24, flexWrap: "wrap", alignItems: "baseline" }}>
        <div>
          <div className="font-mono" style={{ fontSize: 9.5, color: "rgba(255,255,255,0.45)", marginBottom: 4 }}>YOUR PORTFOLIO MIGHT DROP</div>
          <div className="font-serif" style={{ fontSize: 26, fontWeight: 600, color: "var(--coral)", transition: "all .3s var(--ease)" }}>~{portfolioDrop.toFixed(0)}% <span style={{ fontSize: 14, color: "rgba(255,255,255,0.6)" }}>(−{fmt(lost)})</span></div>
        </div>
        <div>
          <div className="font-mono" style={{ fontSize: 9.5, color: "rgba(255,255,255,0.45)", marginBottom: 4 }}>YOU&rsquo;D HAVE ROUGHLY</div>
          <div className="font-serif" style={{ fontSize: 26, fontWeight: 600, color: "#fff", transition: "all .3s var(--ease)" }}>{fmt(remaining)}</div>
        </div>
      </div>
      <p style={{ margin: "16px 0 0", fontSize: 11.5, lineHeight: 1.5, color: "rgba(255,255,255,0.5)" }}>A rough estimate based on your portfolio&rsquo;s risk level — not a prediction. Markets have historically recovered over time.</p>
    </div>
  );
}

function ReassurePanel({ reassure, worstThing }: { reassure: string; worstThing?: string }) {
  const [open, setOpen] = useState(false);
  const parts = reassure.split("|").map((s) => s.trim()).filter(Boolean);
  return (
    <div className="tile fade-up" style={{ marginBottom: "var(--sp-5)", overflow: "hidden", borderColor: "var(--accent)" }}>
      <button
        onClick={() => setOpen((o) => !o)}
        className="font-sans"
        style={{ width: "100%", display: "flex", alignItems: "center", gap: 12, padding: "16px 20px", background: open ? "var(--accent-soft)" : "transparent", border: "none", cursor: "pointer", textAlign: "left", transition: "background .3s var(--ease)" }}
      >
        <span style={{ flexShrink: 0, width: 30, height: 30, borderRadius: 9, background: "var(--accent)", display: "grid", placeItems: "center", color: "#fff", fontSize: 16 }}>🫶</span>
        <span style={{ flex: 1, fontSize: 14.5, fontWeight: 500, color: "var(--ink)" }}>Feeling unsure? Here&rsquo;s the calm read.</span>
        <span style={{ flexShrink: 0, color: "var(--accent)", fontSize: 13, transform: open ? "rotate(180deg)" : "none", transition: "transform .3s var(--ease)" }}>▾</span>
      </button>
      {open && (
        <div className="fade-in" style={{ padding: "4px 20px 20px" }}>
          {worstThing && (
            <div style={{ display: "flex", gap: 10, padding: "12px 14px", marginBottom: 14, borderRadius: "var(--r-md)", background: "var(--paper-2)", border: "1px solid var(--line-soft)" }}>
              <span style={{ flexShrink: 0, fontSize: 14 }}>👉</span>
              <div>
                <div className="font-mono label-soft" style={{ fontSize: 9.5, marginBottom: 3 }}>IF YOU READ NOTHING ELSE</div>
                <span style={{ fontSize: 13.5, lineHeight: 1.5, color: "var(--ink)", fontWeight: 500 }}>{worstThing}</span>
              </div>
            </div>
          )}
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            {parts.map((p, i) => (
              <div key={i} style={{ display: "flex", gap: 10, alignItems: "flex-start" }}>
                <span style={{ flexShrink: 0, marginTop: 2, width: 18, height: 18, borderRadius: "var(--r-pill)", background: i === 1 ? "var(--gold)" : "var(--accent)", color: "#fff", fontSize: 10, display: "grid", placeItems: "center", fontWeight: 600 }}>{i + 1}</span>
                <span style={{ fontSize: 13.5, lineHeight: 1.55, color: "var(--ink-soft)" }}>{p}</span>
              </div>
            ))}
          </div>
        </div>
...
function Results({ meta, holdings, allocation, observations, streaming, done, showEmptyState, sortKey, setSortKey, liveNavs, navLoading, nudge, equityBreakdown, overlap, cost, lastSnapshot }: {
  ...
  overlap: { funds: string[]; text: string } | null; cost: string; lastSnapshot: { at: number; totalValue: string; riskScore?: number; riskLevel?: string; persona?: string; topName?: string; topPct?: number } | null;
}) {
  ...
  return (
    <>
      ...
      {/* Portfolio Memory — what changed since the last analysis (device-local) */}
      {done && meta && lastSnapshot && <MemoryTile prev={lastSnapshot} meta={meta} holdings={holdings} />}

      {/* LAYER 1 — the calm essentials: value + the one thing to watch */}
      {meta && (
        <div className="tile fade-up" style={{ marginBottom: "var(--sp-5)", padding: "20px 24px", display: "flex", gap: "var(--sp-6)", alignItems: "center", flexWrap: "wrap" }}>
          <div style={{ minWidth: 160 }}>
            <div className="font-mono label-soft" style={{ fontSize: 10, marginBottom: 6 }}>TOTAL VALUE</div>
            <div className="font-serif grad-ink" style={{ fontSize: "clamp(24px, 3vw, 32px)", fontWeight: 600, letterSpacing: "-0.02em", lineHeight: 1 }}><CountUpValue value={meta.totalValue} /></div>
          </div>
          {meta.worstThing && (
            <div style={{ flex: 1, minWidth: 240, display: "flex", gap: 10, alignItems: "flex-start" }}>
              <span style={{ flexShrink: 0, fontSize: 15, marginTop: 1 }}>👉</span>
              <div>
                <div className="font-mono label-soft" style={{ fontSize: 10, marginBottom: 4 }}>THE ONE THING TO WATCH</div>
                <span style={{ fontSize: 14, lineHeight: 1.55, color: "var(--ink)", fontWeight: 500 }}>{meta.worstThing}</span>
              </div>
            </div>
          )}
        </div>
      ...
      {/* Streaming status — quiet progress while the full X-Ray builds */}
      {streaming && (
        <div className="fade-in" style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: "var(--sp-5)", color: "var(--ink-faint)", fontSize: 13 }}>
          <span className="spin" style={{ width: 14, height: 14, border: "2px solid var(--line)", borderTopColor: "var(--accent)", borderRadius: "var(--r-pill)", display: "inline-block" }} />
          Reading your portfolio{holdings.length > 0 ? ` — ${holdings.length} holding${holdings.length === 1 ? "" : "s"} found` : "…"}
        </div>
      )}
      ...
      {done && meta && (
        <div style={{ display: "flex", justifyContent: "center", marginBottom: "var(--sp-5)" }}>
          <button
            ...
            className={expanded ? "btn-ghost font-serif" : "btn-primary btn-neon font-serif"}
            style={{ borderRadius: 10, padding: "12px 24px", fontSize: 15, fontWeight: 600, cursor: "pointer" }}
          >
            {expanded ? "Show the simple view ↑" : "See the full X-Ray ↓"}
          </button>
        </div>
      )}

      {expanded && done && (<>
      {/* Anxiety-aware mode: the calm read for a nervous investor */}
      {done && meta?.reassure && <ReassurePanel reassure={meta.reassure} worstThing={meta.worstThing} />}

      {/* Shareable portfolio personality card — distribution built into the product */}
      {done && meta && <PersonaCard meta={meta} />}
      <div className="bento stagger">
      {/* Total value tile */}
      <div className="tile fade-up tile-interactive col-2" style={{ padding: "var(--sp-5)", display: "flex", flexDirection: "column", justifyContent: "center", gap: "var(--sp-3)", minHeight: 128 }}>
      ...
      {/* Summary tile */}
      <div className="tile fade-up tile-interactive col-4" style={{ padding: 24, minHeight: 128 }}>
      ...
      {/* Hidden-overlap detector — reveals funds that secretly hold the same companies */}
      {done && overlap && overlap.text && (
        <div className="tile fade-up col-3" style={{ padding: "18px 20px", borderColor: "var(--coral)", background: "linear-gradient(135deg, #fffaf9 0%, #fff5f3 100%)" }}>
          <div className="font-mono label-strong" style={{ fontSize: 10, color: "var(--coral)", marginBottom: 8 }}>HIDDEN OVERLAP</div>
          {overlap.funds.length >= 2 && (
            <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginBottom: 10 }}>
              {overlap.funds.map((f, i) => (
                <span key={i} style={{ fontSize: 11, padding: "3px 9px", borderRadius: "var(--r-pill)", background: "#fff", border: "1px solid var(--coral)", color: "var(--coral)", maxWidth: 180, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{f}</span>
              ))}
            </div>
          )}
          <p style={{ margin: 0, fontSize: 13.5, lineHeight: 1.5, color: "var(--ink-soft)" }}>{overlap.text}</p>
        </div>
      )}

      {/* What this costs you — fees translated into rupees */}
      {done && cost && (
        <div className="tile fade-up col-3" style={{ padding: "18px 20px", borderColor: "var(--gold)", background: "linear-gradient(135deg, #fffdf7 0%, #fdf8ee 100%)" }}>
          <div className="font-mono label-strong" style={{ fontSize: 10, color: "var(--gold)", marginBottom: 8 }}>WHAT THIS COSTS YOU</div>
          <p style={{ margin: 0, fontSize: 13.5, lineHeight: 1.5, color: "var(--ink-soft)" }}>{cost}</p>
        </div>
      )}

      {/* The Gut Check — makes abstract risk visceral */}
      {done && meta?.crashTest && (
        <div className="tile reveal col-6" style={{ padding: "18px 22px", display: "flex", gap: 14, alignItems: "flex-start", background: "linear-gradient(135deg, #1a1a17 0%, #2a2a24 100%)", border: "none" }}>
          <span style={{ flexShrink: 0, width: 30, height: 30, borderRadius: 9, background: "rgba(255,255,255,0.1)", display: "grid", placeItems: "center", fontSize: 16 }}>📉</span>
          <div>
            <div className="font-mono" style={{ fontSize: 10, letterSpacing: "0.12em", color: "rgba(255,255,255,0.55)", marginBottom: 5 }}>THE GUT CHECK · IF THE MARKET CRASHED</div>
            <p style={{ margin: 0, fontSize: 14, lineHeight: 1.55, color: "rgba(255,255,255,0.92)" }}>{meta.crashTest}</p>
          </div>
        </div>
      )}

      {/* Interactive stress test — drag to feel the risk */}
      {done && meta && parseValue(meta.totalValue) > 0 && <CrashSlider totalValue={meta.totalValue} riskScore={meta.riskScore} />}
