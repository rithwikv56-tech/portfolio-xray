...
// Fetch with a timeout and one retry — free public APIs are flaky, so don't give up on the first hiccup.
// The User-Agent header matters: some public APIs start returning 403 to requests without one.
async function fetchResilient(url: string, timeoutMs = 8000, retries = 1): Promise<Response> {
  for (let attempt = 0; attempt <= retries; attempt++) {
    ...
    try {
      const res = await fetch(url, {
        ...
        headers: { "Accept": "application/json", "User-Agent": "PortfolioXray/1.0" },
      });
      clearTimeout(timer);
      if (res.ok) return res;
      if (res.status < 500) return res; // 4xx won't fix itself on retry
    } catch {
      clearTimeout(timer);
    }
...
async function getSchemes(): Promise<SchemeRef[]> {
  ...
  const res = await fetchResilient(`${MFAPI_BASE}/mf`, 12000, 2);
  ...
  try {
    ...
    const results = await Promise.all(
      wanted.map(async (name) => {
        ...
        try {
          const r = await fetchResilient(`${MFAPI_BASE}/mf/${match.schemeCode}/latest`, 8000, 1);
